...
// ... (Componente SalesProofPopup - sem alterações)
const SalesProofPopup = ({ show, onClose }: { show: boolean; onClose: () => void }) => {
  ...
  return (
    <motion.div
      ...
      style={{ fontSize: "13px", fontFamily: "Inter, -apple-system, BlinkMacSystemFont, sans-serif" }}
    >
...
export default function SigiloX() {
  ...
  const [stepCompleted, setStepCompleted] = useState({ profilePhotos: false, conversations: false, finalizing: false })
  ...
  const handleCreatePayment = async () => {
    ...
    const selectedBumpIds = Object.keys(selectedBumps).filter((key) => selectedBumps[key as keyof typeof selectedBumps])
    try {
      ...
      if (!response.ok || data.hasError) throw new Error(data.error || "Não foi possível gerar o pagamento.")
  ...
  useEffect(() => {
    ...
    if (debouncedPhone) fetchWhatsAppPhoto()
  }, [debouncedPhone])

  const handlePhoneChange = (value: string) => {
    let formattedValue = value
    if (!value.startsWith(selectedCountry.code)) {
      if (value && !value.startsWith("+")) formattedValue = `${selectedCountry.code} ${value}`
      else if (value.startsWith("+") && !value.startsWith(selectedCountry.code)) formattedValue = value
      else formattedValue = `${selectedCountry.code} ${value.replace(selectedCountry.code, "").trim()}`
    }
    setPhoneNumber(formattedValue)
  }
  useEffect(() => {
    const handleClickOutside = (event: MouseEvent) => {
      if (showCountryDropdown && !(event.target as Element).closest(".relative")) setShowCountryDropdown(false)
    }
  ...
  // ... (O resto das funções e useEffects permanece igual)
    ...
    <div className="min-h-screen" style={{ fontFamily: "Inter, -apple-system, BlinkMacSystemFont, sans-serif" }}>
              ...
              {getProgressSteps().map((step, index) => (
                <div key={step.id} className="flex items-center">
                  <div className="stepper-step flex items-center gap-2 min-w-[80px] sm:min-w-[100px]">
                    <div
                      className={`w-7 h-7 sm:w-8 sm:h-8 rounded-full flex items-center justify-center text-xs font-bold transition-all duration-300 flex-shrink-0 ${step.completed ? "bg-gradient-to-r from-green-500 to-green-600 text-white shadow-lg" : "bg-gray-200 text-gray-500"}`}
                    >
                      {step.completed ? "✓" : index + 1}
                    </div>
                    <span
                      className={`font-medium transition-colors duration-300 text-xs sm:text-sm whitespace-nowrap ${step.completed ? "text-green-600" : "text-gray-500"}`}
                    >
      ...
      <div className={currentStep !== "landing" ? "pt-16 sm:pt-20" : ""}>
        <AnimatePresence mode="wait">
            ...
            >
              ...
              <div className="relative z-10 container mx-auto px-4 py-8 sm:py-12">
                <div className="text-center mb-12 sm:mb-16">
                  ...
                  >
                    Pare de perder o sono se perguntando se a pessoa ainda está usando aplicativos de namoro. Obtenha as
                    respostas que você precisa - de forma completamente anônima.
                  </motion.p>
                  <motion.div
                    initial={{ y: 20, opacity: 0 }}
                    animate={{ y: 0, opacity: 1 }}
                ...
                >
                  <div className="flex items-center gap-3 sm:gap-4 bg-white/10 backdrop-blur-sm text-white px-4 sm:px-6 py-3 sm:py-4 rounded-2xl border border-white/20 hover:bg-white/15 transition-all duration-300">
                    ...
                    <span className="font-semibold text-sm sm:text-base">
                      ✅ Veja o último login (mesmo quando dizem que 'não usam mais' apps)
                    </span>
                  </div>
                  <div className="flex items-center gap-3 sm:gap-4 bg-white/10 backdrop-blur-sm text-white px-4 sm:px-6 py-3 sm:py-4 rounded-2xl border border-white/20 hover:bg-white/15 transition-all duration-300">
                    <MapPin className="w-5 h-5 sm:w-6 sm:h-6 flex-shrink-0 text-[#00FF99]" />
                    <span className="font-semibold text-sm sm:text-base">
                      ✅ Descubra de onde a pessoa está realmente usando os aplicativos
                    </span>
                  </div>
                  <div className="flex items-center gap-3 sm:gap-4 bg-white/10 backdrop-blur-sm text-white px-4 sm:px-6 py-3 sm:py-4 rounded-2xl border border-white/20 hover:bg-white/15 transition-all duration-300">
                    <Eye className="w-5 h-5 sm:w-6 sm:h-6 flex-shrink-0 text-[#00FF99]" />
                    <span className="font-semibold text-sm sm:text-base">
                      ✅ Acesse as conversas que não querem que você veja
                    </span>
                  </div>
                  <div className="flex items-center gap-3 sm:gap-4 bg-white/10 backdrop-blur-sm text-white px-4 sm:px-6 py-3 sm:py-4 rounded-2xl border border-white/20 hover:bg-white/15 transition-all duration-300">
                    <Shield className="w-5 h-5 sm:w-6 sm:h-6 flex-shrink-0 text-[#00FF99]" />
                    <span className="font-semibold text-sm sm:text-base">
                      ✅ Sua investigação permanece completamente privada
                    </span>
                  </div>
                </motion.div>
                <motion.div
                ...
                >
                  ...
                  >
                    <span className="block text-center leading-tight px-2 break-words whitespace-normal">
                      🔍 DESCUBRA A VERDADE – INICIAR BUSCA ANÔNIMA
                    </span>
                  </Button>
                  <p className="text-sm text-gray-300 mt-4 font-medium">
                    Investigação 100% anônima. A pessoa nunca saberá que você verificou.
                  </p>
                </motion.div>
              </div>
                ...
                <div className="container mx-auto px-4">
                  <div className="text-center mb-8 sm:mb-12">
                    <h2 className="text-xl sm:text-2xl md:text-3xl font-bold text-[#333333] mb-4">
                      Você Não Está Sendo Paranoico(a) -
                    </h2>
                    ...
                    <p className="text-gray-600 text-base sm:text-lg max-w-2xl mx-auto leading-relaxed">
                      Pare de duvidar dos seus instintos. Obtenha a clareza necessária para tomar decisões informadas
                  ...
                  <div className="grid grid-cols-2 gap-4 sm:gap-6 md:gap-8 max-w-4xl mx-auto mb-8 sm:mb-12">
                    <div className="text-center p-4 sm:p-6 bg-white rounded-2xl shadow-lg hover:shadow-xl transition-all duration-300">
                      ...
                      <p className="text-xs sm:text-sm text-gray-600">
                        Veja quando usaram apps de namoro pela última vez
                      </p>
                    </div>
                    <div className="text-center p-4 sm:p-6 bg-white rounded-2xl shadow-lg hover:shadow-xl transition-all duration-300">
                      ...
                      <p className="text-xs sm:text-sm text-gray-600">Onde estiveram usando os apps</p>
                    </div>
                    <div className="text-center p-4 sm:p-6 bg-white rounded-2xl shadow-lg hover:shadow-xl transition-all duration-300">
                      <div className="w-12 h-12 sm:w-16 sm:h-16 bg-gradient-to-r from-red-100 to-red-200 rounded-2xl flex items-center justify-center mx-auto mb-3 sm:mb-4">
                      ...
                      <p className="text-xs sm:text-sm text-gray-600">Fotos que não querem que você veja</p>
                    </div>
                    <div className="text-center p-4 sm:p-6 bg-white rounded-2xl shadow-lg hover:shadow-xl transition-all duration-300">
                      ...
                      <h4 className="font-bold text-[#333333] mb-2 text-sm sm:text-base">CONVERSAS PRIVADAS</h4>
                      <p className="text-xs sm:text-sm text-gray-600">O que realmente estão dizendo a outras pessoas</p>
                    </div>
                  </div>
                  <div className="text-center mb-8 sm:mb-12">
                    <h3 className="text-lg sm:text-2xl md:text-3xl font-bold text-[#333333] mb-6 sm:mb-8 px-2">
                      Você Não Está Sozinho(a) - Veja o que Outros Descobriram
                    </h3>
                    <div className="max-w-3xl mx-auto space-y-5 sm:space-y-6 mb-6 sm:mb-8">
                      <div className="testimonial-card bg-white rounded-xl shadow-lg p-4 sm:p-5 flex items-start gap-4">
                        ...
                        <div className="flex-1 min-w-0 text-left">
                          <div className="mb-2">
                            <p className="font-bold text-[#333333] text-base sm:text-lg">Laura, 32</p>
                          ...
                          <div className="mb-3">
                            ...
                            <p className="text-[#444444] text-base sm:text-lg leading-relaxed font-normal">
                              Eu sabia que algo estava errado. O relatório confirmou meus piores medos, mas pelo menos
                              agora eu podia tomar uma decisão informada em vez de viver em constante ansiedade.
                            </p>
                          </div>
                      ...
                      <div className="testimonial-card bg-white rounded-xl shadow-lg p-4 sm:p-5 flex items-start gap-4">
                        ...
                        <div className="flex-1 min-w-0 text-left">
                          <div className="mb-2">
                            <p className="font-bold text-[#333333] text-base sm:text-lg">Fernanda, 28</p>
                            <p className="text-xs sm:text-sm text-blue-600 font-medium">
                              Investigação concluída em Junho de 2025
                            </p>
                          </div>
                          <div className="mb-3">
                            <svg
                      ...
                      <div className="testimonial-card bg-white rounded-xl shadow-lg p-4 sm:p-5 flex items-start gap-4">
                        ...
                        <div className="flex-1 min-w-0 text-left">
                          <div className="mb-2">
                            <p className="font-bold text-[#333333] text-base sm:text-lg">Beatriz, 35</p>
                            <p className="text-xs sm:text-sm text-green-600 font-medium">✓ Usuária Verificada</p>
                          </div>
                          <div className="mb-3">
                            <svg
                            ...
                            <p className="text-[#444444] text-base sm:text-lg leading-relaxed font-normal">
                              Eu me senti culpada por verificar, mas meus instintos estavam certos. Agora posso seguir
                              em frente com confiança em vez de viver na dúvida.
                            </p>
                          </div>
                          <div className="flex items-center text-[#FFD700] text-sm sm:text-base gap-1">
                            <span>⭐⭐⭐⭐⭐</span>
                    ...
                    >
                      <span className="block text-center leading-tight px-2 break-words whitespace-normal">
                        🔍 INICIAR MINHA INVESTIGAÇÃO ANÔNIMA
                      </span>
                    </Button>
                  </div>
                  <div className="text-center px-4">
                    <p className="text-xs text-gray-500 flex items-center justify-center gap-2 font-medium">
                      ...
                      100% anônimo - Sua investigação permanece completamente privada
                    </p>
                  </div>
                </div>
              </div>
                ...
                <div className="w-full max-w-lg">
                  <div className="text-center mb-6 sm:mb-8">
                    ...
                    <h1 className="text-xl sm:text-2xl md:text-3xl font-bold text-white mb-3 sm:mb-4">
                      🔍 Ajude-nos a Encontrar o que Estão Escondendo
                    </h1>
                    <p className="text-gray-200 text-sm sm:text-base px-4 leading-relaxed">
                      Quanto mais detalhes você fornecer, mais fundo podemos investigar. Tudo permanece 100% anônimo.
                    </p>
                  </div>
                    ...
                    <CardContent className="p-4 sm:p-8 space-y-6 sm:space-y-8">
                      <div>
                        <label className="block text-sm sm:text-base font-semibold text-[#333333] mb-3 sm:mb-4">
                          Envie a Foto para Reconhecimento Facial
                        </label>
                        ...
                        <p className="text-xs sm:text-sm text-gray-500 mt-3 font-medium">
                          Vamos escanear todas as plataformas de namoro para encontrar perfis correspondentes.
                        </p>
                      </div>
                      <div>
                        <label className="block text-sm sm:text-base font-semibold text-[#333333] mb-2 sm:mb-3">
                          Número de WhatsApp que a pessoa usa
                        </label>
                        ...
                        <p className="text-xs sm:text-sm text-gray-500 mt-2 font-medium">
                          Isso nos ajuda a rastrear a atividade do dispositivo e cruzar com os padrões de uso de
                            ...
                            <div className="flex items-center gap-3 sm:gap-4">
                              ...
                              <div className="flex-1">
                                <p className="font-semibold text-[#333333] text-sm sm:text-base">
                                  Perfil do WhatsApp Encontrado
                                </p>
                                <p className="text-xs sm:text-sm text-gray-600">
                                  {isPhotoPrivate ? "Foto privada detectada" : "Foto do perfil carregada"}
                                </p>
                              </div>
                      ...
                      <div>
                        ...
                        <div className="grid grid-cols-3 gap-2 sm:gap-3">
                          {[
                            { value: "masculino", label: "Homem", icon: "👨" },
                            { value: "feminino", label: "Mulher", icon: "👩" },
                            { value: "nao-binario", label: "Não-binário", icon: "🧑" },
                          ].map((option) => (
                            <button
                              key={option.value}
                              type="button"
                              onClick={() => setSelectedGender(option.value)}
                              className={`p-3 sm:p-4 rounded-xl border-2 transition-all duration-200 text-center ${selectedGender === option.value ? "border-blue-500 bg-blue-50 text-blue-700" : "border-gray-200 hover:border-gray-300"}`}
                            >
                      ...
                      <div>
                        ...
                        <div className="grid grid-cols-2 gap-2 sm:gap-3">
                          {[
                            ...
                            { value: "45-54", label: "45+ anos" },
                          ].map((option) => (
                            <button
                              key={option.value}
                              type="button"
                              onClick={() => setAgeRange(option.value)}
                              className={`p-3 sm:p-4 rounded-xl border-2 transition-all duration-200 text-center ${ageRange === option.value ? "border-blue-500 bg-blue-50 text-blue-700" : "border-gray-200 hover:border-gray-300"}`}
                            >
                              <div className="text-xs sm:text-sm font-medium">{option.label}</div>
                            </button>
                          ))}
                        ...
                        <p className="text-xs sm:text-sm text-gray-500 mt-2 font-medium">
                          Isso nos ajuda a refinar os parâmetros de busca em todas as plataformas de namoro.
                        </p>
                      </div>
                      <div>
                        <label className="block text-sm sm:text-base font-semibold text-[#333333] mb-3 sm:mb-4">
                        ...
                        <div className="space-y-2 sm:space-y-3">
                          {[
                            { value: "week", label: "Na última semana", desc: "(mudanças de comportamento recentes)" },
                            {
                              value: "month",
                              label: "No último mês",
                              desc: "(distanciamento gradual/escondendo o celular)",
                            },
                            { value: "longer", label: "Há mais de um mês", desc: "(sentimento contínuo)" },
                            { value: "sure", label: "Só preciso ter certeza", desc: "" },
                          ].map((option) => (
                            <button
                              key={option.value}
                              type="button"
                              onClick={() => setLastTinderUse(option.value)}
                              className={`w-full p-3 sm:p-4 rounded-xl border-2 transition-all duration-200 text-left ${lastTinderUse === option.value ? "border-blue-500 bg-blue-50 text-blue-700" : "border-gray-200 hover:border-gray-300"}`}
                            >
                      ...
                      <div>
                        <label className="block text-sm sm:text-base font-semibold text-[#333333] mb-3 sm:mb-4">
                          A pessoa tem "trabalhado até tarde" ou viajado?
                        </label>
                        <div className="space-y-2 sm:space-y-3">
                          {[
                            {
                              value: "yes",
                              label: "Sim",
                              desc: '"Novas demandas de trabalho" ou viagens inexplicadas',
                            },
                            { value: "no", label: "Não", desc: "Mudanças de comportamento aconteceram em casa" },
                            { value: "unknown", label: "Não sei", desc: "A pessoa é reservada sobre a agenda" },
                          ].map((option) => (
                            <button
                              key={option.value}
                              type="button"
                              onClick={() => setCityChange(option.value)}
                              className={`w-full p-3 sm:p-4 rounded-xl border-2 transition-all duration-200 text-left ${cityChange === option.value ? "border-blue-500 bg-blue-50 text-blue-700" : "border-gray-200 hover:border-gray-300"}`}
                            >
                              <div className="font-medium text-sm sm:text-base">{option.label}</div>
                              <div className="text-xs sm:text-sm text-gray-500 mt-1">{option.desc}</div>
                            </button>
                      ...
                      <div className="text-center">
                        <p className="text-xs sm:text-sm text-gray-500 flex items-center justify-center gap-2 font-medium">
                          ...
                          Seus dados são criptografados e excluídos automaticamente após 24 horas
                        </p>
                      </div>
                    </CardContent>
                  </Card>
              ...
              <div className="w-full max-w-md">
                <Card className="bg-white rounded-2xl shadow-2xl border-0 overflow-hidden">
                  <CardContent className="p-6 sm:p-8 text-center">
                    ...
                    <div className="space-y-3 sm:space-y-4 mb-6 sm:mb-8">
                      <div
                        className={`flex items-center gap-3 sm:gap-4 p-3 sm:p-4 rounded-xl ${stepCompleted.profilePhotos ? "bg-green-50" : "bg-blue-50"}`}
                      >
                      ...
                      <div
                        className={`flex items-center gap-3 sm:gap-4 p-3 sm:p-4 rounded-xl ${stepCompleted.conversations ? "bg-green-50" : stepCompleted.profilePhotos ? "bg-blue-50" : "bg-gray-50"}`}
                      >
                        ...
                        <span
                          className={`text-xs sm:text-sm font-medium ${stepCompleted.conversations || stepCompleted.profilePhotos ? "text-gray-700" : "text-gray-500"}`}
                        >
                          Processando conversas...
                        </span>
                      </div>
                      <div
                        className={`flex items-center gap-3 sm:gap-4 p-3 sm:p-4 rounded-xl ${stepCompleted.finalizing ? "bg-green-50" : stepCompleted.conversations ? "bg-blue-50" : "bg-gray-50"}`}
                      >
                        ...
                        <span
                          className={`text-xs sm:text-sm font-medium ${stepCompleted.finalizing || stepCompleted.conversations ? "text-gray-700" : "text-gray-500"}`}
                        >
              ...
              <div className="container mx-auto max-w-4xl">
                    ...
                    <div className="flex items-center gap-2 sm:gap-3">
                      ...
                      <div>
                        <h3 className="font-bold text-sm sm:text-base">
                          🚨 PERFIL ENCONTRADO - ESTÃO ATIVOS NO TINDER
                        </h3>
                        <p className="text-xs sm:text-sm opacity-90">Visto por último: Online agora</p>
                      </div>
                    </div>
                  </div>
                    ...
                    <div className="flex items-center gap-2 sm:gap-3">
                      ...
                      <div>
                        ...
                        <p className="text-xs sm:text-sm opacity-90">
                          Confirmamos que este número está vinculado a um perfil ATIVO no Tinder. Registros de uso
                          recentes detectados em {city || "sua área"}.
                        </p>
                      </div>
                    </div>
                  </div>
                ...
                <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 sm:gap-4 mb-6 sm:mb-8">
                  <div className="bg-white rounded-xl p-3 sm:p-4 text-center shadow-lg">
                    ...
                    <div className="text-xs sm:text-sm text-gray-600 font-medium">MATCHES (7 DIAS)</div>
                  </div>
                  <div className="bg-white rounded-xl p-3 sm:p-4 text-center shadow-lg">
                    <div className="text-xl sm:text-2xl font-bold text-orange-500 mb-1">30</div>
                    <div className="text-xs sm:text-sm text-gray-600 font-medium">CURTIDAS (7 DIAS)</div>
                  </div>
                  <div className="bg-white rounded-xl p-3 sm:p-4 text-center shadow-lg">
                      ...
                      {generatedProfiles.map((profile, index) => (
                        ...
                        >
                          ...
                          <div className="flex-1">
                            ...
                            <p className="text-sm text-gray-600">Visto por último: {profile.lastSeen}</p>
                            <p className="text-sm text-green-600">Chat ativo: frequentemente online</p>
                          </div>
                        </div>
                      ))}
                    </div>
                ...
                <Card className="bg-white rounded-2xl shadow-lg border-0 mb-6 sm:mb-8">
                  <CardContent className="p-4 sm:p-6">
                    ...
                    <p className="text-sm sm:text-base text-gray-600 mb-4 sm:mb-6">
                      Veja todas as fotos do perfil (incluindo as que você nunca viu)
                    </p>
                    <div className="relative">
                      ...
                      <div className="flex justify-center gap-2 mt-4">
                        {blockedImages.map((_, index) => (
                          <button
                            ...
                            className={`w-2 h-2 sm:w-3 sm:h-3 rounded-full transition-colors ${index === currentSlide ? "bg-blue-500" : "bg-gray-300"}`}
                          />
                        ))}
                      </div>
                    </div>
                ...
                <Card className="bg-white rounded-2xl shadow-lg border-0">
                  <CardContent className="p-4 sm:p-6 text-center">
                    <div className="mb-4 sm:mb-6">
                      ...
                      <h3 className="text-xl sm:text-2xl md:text-4xl font-bold text-[#333333] mb-3 sm:mb-4">
                        🔓 DESBLOQUEAR RELATÓRIO COMPLETO
                      </h3>
                      <p className="text-sm sm:text-base text-gray-600 mb-4 sm:mb-6">
                        Tenha acesso instantâneo ao relatório completo com fotos sem censura e histórico completo de
                        conversas.
                      </p>
                    </div>
                    <div className="bg-gradient-to-r from-red-500 to-red-600 text-white p-4 sm:p-6 rounded-xl shadow-lg mb-4 sm:mb-6">
                      <div className="flex items-center justify-center gap-2 sm:gap-3 mb-3">
                        ...
                        <span className="font-bold text-lg sm:text-xl">O RELATÓRIO SERÁ APAGADO EM:</span>
                      </div>
                      ...
                      <p className="text-sm sm:text-base text-center leading-relaxed opacity-90">
                        Após o tempo expirar, este relatório será permanentemente excluído por razões de privacidade.
                        Esta oferta não pode ser recuperada posteriormente.
                      </p>
                    </div>
                    <Button
          ...
          {/* Offer Page - CORRIGIDA E FUNCIONAL */}
          {currentStep === "offer" && (
            <motion.div
              key="offer"
              initial={{ opacity: 0 }}
                    ...
                    {!paymentResult ? (
                      <>
                        <div className="mb-6 sm:mb-8">
                          ...
                          <p className="text-base sm:text-lg text-gray-600 mb-4 sm:mb-6 leading-relaxed">
                            Pare de se perguntar. Obtenha todos os detalhes - de forma totalmente confidencial.
                          </p>
                          <div className="bg-gradient-to-r from-red-50 to-pink-50 border-2 border-red-200 rounded-xl p-4 sm:p-6">
                            <p className="text-sm sm:text-base text-red-700 font-semibold leading-relaxed">
                              Seus instintos estavam certos. Agora veja exatamente o que eles têm escondido enquanto
                              olham nos seus olhos e mentem.
                            </p>
                          </div>
                        </div>
                        <div className="mb-6 sm:mb-8">
                        ...
                        <div className="text-left mb-6 sm:mb-8">
                          ...
                          <div className="space-y-3 sm:space-y-4">
                            <div className="flex items-start gap-3 sm:gap-4">
                              <CheckCircle className="w-5 h-5 sm:w-6 sm:h-6 text-green-500 flex-shrink-0 mt-1" />
                              <span>Todas as Fotos do Perfil (incluindo as que eles acham que você nunca verá)</span>
                            </div>
                            <div className="flex items-start gap-3 sm:gap-4">
                              <CheckCircle className="w-5 h-5 sm:w-6 sm:h-6 text-green-500 flex-shrink-0 mt-1" />
                              <span>
                                Histórico Completo de Conversas (veja exatamente o que estão dizendo a outras pessoas)
                              </span>
                            </div>
                          </div>
                        </div>
                        <div className="text-center mb-6 sm:mb-8">
                          ...
                          <div className="space-y-4">
                            {orderBumps.map((bump) => (
                              <div
                                key={bump.id}
                                onClick={() => handleBumpToggle(bump.id as "whats" | "insta" | "facebook" | "gps")}
                                className={`p-4 border-2 rounded-xl flex items-center gap-4 cursor-pointer transition-all duration-300 ${selectedBumps[bump.id as keyof typeof selectedBumps] ? "border-green-500 bg-green-50 shadow-md" : "border-gray-200 hover:border-gray-300"}`}
                              >
                                <img
                                ...
                                <div className="text-right">
                                  ...
                                  <div className="mt-2 flex justify-end">
                                    <div
                                      className={`w-6 h-6 rounded-md flex items-center justify-center transition-colors ${selectedBumps[bump.id as keyof typeof selectedBumps] ? "bg-green-500" : "border border-gray-400 bg-white"}`}
                                    >
                                      {selectedBumps[bump.id as keyof typeof selectedBumps] && (
                          ...
                          <div className="space-y-4">
                            ...
                            <div>
                              ...
                              <p className="text-xs text-gray-500 mt-1">
                                Enviaremos o acesso ao seu relatório neste e-mail.
                              </p>
                            </div>
                            <div>
                        ...
                        <Button
                          ...
                          disabled={isLoadingPayment || !canPay}
                      ...
                      <div>
                        <h2 className="text-2xl font-bold text-gray-800 mb-4">
                          Quase lá! Pague com PIX para liberar seu acesso.
                        </h2>
                        ...
                        <div className="mt-6">
                          ...
                          <div className="mt-1 flex rounded-md shadow-sm">
                            <input
                              ...
                              className="flex-1 block w-full rounded-none rounded-l-md sm:text-sm border-gray-300 p-2 bg-gray-100"
                            />
                            <button
                              onClick={() => navigator.clipboard.writeText(paymentResult.pix.payload)}
                        ...
                        <p className="mt-8 text-sm text-gray-500">
                          Após a confirmação do pagamento, você receberá um e-mail com o acesso ao seu relatório
                          completo.
                        </p>
                      </div>
                    )}
